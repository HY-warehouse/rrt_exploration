...
def node():
	...
	info_radius= rospy.get_param('~info_radius',20.0)					#this can be smaller than the laser scanner range, >> smaller >>less computation time>> too small is not good, info gain won't be accurate
	info_multiplier=rospy.get_param('~info_multiplier',15.0)		#到边界点的预期行进距离
	hysteresis_radius=rospy.get_param('~hysteresis_radius',30.0)			#at least as much as the laser scanner range
	hysteresis_gain=rospy.get_param('~hysteresis_gain',3.0)				#bigger than 1 (biase robot to continue exploring current region
	frontiers_topic= rospy.get_param('~frontiers_topic','/filtered_points')	
	n_robots = rospy.get_param('~n_robots',1)
	namespace = rospy.get_param('~namespace','')
	namespace_init_count = rospy.get_param('namespace_init_count',1)
	delay_after_assignement=rospy.get_param('~delay_after_assignement',0.2)
...
#wait if map is not received yet
	...
	if len(namespace)>0:
		...
		# 机器人列表，增加机器人时，需要在这里更改
		robot_list = ['robot_1', 'robot_2', 'robot_3', 'robot_4']

		for i in range(0,n_robots):

			# rospy.loginfo('namespace:  {}'.format(namespace))
			# 输出为 namespace:  robot_
			# rospy.loginfo('namespace+str(i+namespace_init_count):  {}'.format(namespace+str(i+namespace_init_count)))
			# 输出为 namespace+str(i+namespace_init_count):  robot_1

			robots.append(robot(robot_list[i]))

	elif len(namespace)==0:
			robots.append(robot(namespace))
	for i in range(0,n_robots):
		robots[i].sendGoal(robots[i].getPosition())
		rospy.loginfo('robot '+str(i)+' sendGoal '+str(robots[i].getPosition()))
#-------------------------------------------------------------------------
#---------------------     Main   Loop     -------------------------------
#-------------------------------------------------------------------------
...
#get number of available/busy robots
		...
		for i in range(0,n_robots):
			if (robots[i].getState()==1):
				nb.append(i)
				# rospy.loginfo("robot "+str(i)+"'s state is "+str(robots[i].getState()))
			else:
				na.append(i)	
				# rospy.loginfo("robot "+str(i)+"'s state is "+str(robots[i].getState()))
