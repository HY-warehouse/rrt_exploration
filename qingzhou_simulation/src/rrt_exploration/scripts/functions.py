...
class robot:
    ...
    def __init__(self, name):
        ...
        #  waitForTransform() 传入的第一个参数是 目标坐标系 , 第二个参数是 源坐标系 , 第三个参数是时间戳 , 第四个参数是等待时间
        ...
        while cond == 0:
            try:
                ...
                # rospy.loginfo('self.global_frame: {}, self.name: {}, self.robot_frame: {}'.format(self.global_frame, self.name,self.robot_frame))
                # rospy.loginfo('cond: {}'.format(cond))
        ...
        # rospy.loginfo('self.client.wait_for_server()')

        robot.goal.target_pose.header.frame_id = self.global_frame
        # rospy.loginfo('test')
        robot.goal.target_pose.header.stamp = rospy.Time.now()
        # rospy.loginfo('test')
        # rospy.loginfo('self.name: {}, self.plan_service: {}'.format(self.name, self.plan_service))
    ...
    def sendGoal(self, point):
        ...
        robot.goal.target_pose.header.frame_id = self.name+'/map'

        rospy.loginfo('self.global_frame: {}'.format(self.global_frame))
        rospy.loginfo('self.robot_frame: {}'.format(self.robot_frame))
        rospy.loginfo('robot.goal: {}'.format(robot.goal))
