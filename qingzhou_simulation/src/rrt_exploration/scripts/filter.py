...
#         rospy.loginfo(data._connection_header['topic'][litraIndx])

#         indx = int((data._connection_header['topic']
#                    [litraIndx]).replace('_', '0'))-namespace_init_count
...
def globalMap(data):
    ...
    # rospy.loginfo("str(data._connection_header['topic']).split('/'): {}".format(topic_breakdownlist ) )
    # rospy.loginfo("n_robots: {}".format(n_robots) )

    indx = 0
    for ia in range(0, n_robots):
        if robot_list[ia] in topic_breakdownlist:
            indx = ia

            # rospy.loginfo("indx: {}".format(indx) )

    globalmaps[indx] = data

    # 测试用
    # rospy.loginfo('---------------------------------')
    # rospy.loginfo('globalmaps回调函数')
    # rospy.loginfo('globalmaps[0]: {}'.format(len(globalmaps[0].data)))
    # rospy.loginfo('globalmaps[1]: {}'.format(len(globalmaps[1].data)))
    # rospy.loginfo('globalmaps[2]: {}'.format(len(globalmaps[2].data)))
    # rospy.loginfo('len(globalmaps): {}'.format(len(globalmaps)))
    # rospy.loginfo('globalmaps回调函数end')
    # rospy.loginfo('---------------------------------')
...
def node():
    ...
    info_radius = rospy.get_param('~info_radius', 25.0)
...
# wait if any of robots' global costmap map is not received yet
    for i in range(1, n_robots):
        ...
        while (len(globalmaps[i].data) < 1):
            ...
            # 调试用
            # rospy.loginfo('len(globalmaps[i].data)')
            # rospy.loginfo(len(globalmaps[i].data))
            # rospy.loginfo('globalmaps[i].data')
            # rospy.loginfo(globalmaps[i].data)
            # rospy.loginfo('globalmaps[i]')
            # rospy.loginfo(globalmaps[i])
            # rospy.loginfo('---------------------------------')
        ...
        # rospy.loginfo('globalmaps[2]: {}'.format(len(globalmaps[2].data)))
        # rospy.loginfo('len(globalmaps): {}'.format(len(globalmaps)))
        # rospy.loginfo('len(globalmaps[i].data): {}'.format(len(globalmaps[i].data)))
        # rospy.loginfo('---------------------------------')
