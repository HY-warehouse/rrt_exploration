...
class OdometryNode:
    # Set publishers
    pub_odom = rospy.Publisher('/robot_2/odom', Odometry, queue_size=1)
    ...
    def sub_robot_pose_update(self, msg):
        ...
        try:
            arrayIndex = msg.name.index('robot_2::base_link')
            ...
            # rospy.loginfo("--------------------")
            # rospy.loginfo("msg.name: %s", msg.name)
            # rospy.loginfo("robot_2--self.last_received_pose: %s", self.last_received_pose)
            # rospy.loginfo("arrayIndex: %s", arrayIndex)
            # rospy.loginfo("--------------------")
    ...
    def timer_callback(self, event):
        ...
        cmd.header.frame_id = 'robot_2/odom'
        cmd.child_frame_id = 'robot_2/base_link'
...
if __name__ == '__main__':
    rospy.init_node("gazebo_odometry_node_robot2")
    node = OdometryNode()
    rospy.spin()
